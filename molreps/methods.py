"""
Molecular feature representation based on atom, coordinates. A loose collection of functions.
Modular functions to compute distance matrix, angles and coordinates, bond table etc.
Many functions are written for last dimension and work for batches too. 
Ideally all functions are vectorized.
...
def element_list_to_value(elem_list, replace_dict):
    ...
    @TODO: use itertools to speed up
...
def get_bondtable_from_inversedistancematrix(invdistmat,elements,radii_dict=None, k1=16.0, k2=4.0/3.0, cutoff=0.85,force_bonds=True): 
    """
    Get bonding table from inverse distance matrix defined at last dimensions (...,N,N) and corresponding bond-radii. Keeps shape with (...,N,N).
    ...
    Args:
        ...
        elements (list):            A list of elements as string is expected matching the invdistmat (...,N),
                                    for which the radii are computed.
        radii_dict (dictionary):    covalent radii for each element. If default=None, stored values are used. Otherwise dictionary with covalent bonding radii
                                    example: {'H' : 0.24, 'He': 0.46, 'Li': 1.2, ...}
        k1 (value):                 default = 16
...
def get_angles(coords,ind1,ind2a,ind2b):
    """
    Compute angeles between coordinates (...,N,3) from a matching index list.
    Angles are between ind1<(ind2a,ind2b) taking coords[ind].
    
    Args:
        coords (numpy array): list of coordinates of points (...,N,3)    
        ind1 (numpy array): index list of start point (...,M) that means coords[i]
        ind2a (numpy array): index list of end point of first bonding partner (...,M)
        ind2b (numpy array): index list of start point of second bonding partner (...,M)
    
    Returns:
        angle_cos (numpy array): cos() of the angles between ind1<(ind2a,ind2b)
    ...
    return angle_cos, angles ,norm_vec1,norm_vec2
...
def make_angle_list(ind1,ind2):
    ...
    Vectorized but requires memory for all connections N_bonds*N_atoms. Uses masking
    ...
    Returns:
            index1 (numpy array): Indexlist of start index for a bond-tuple. Shape (M,)
            index2 (numpy array): Indexlist of two end indeces between which an angle can be defined. Shape (M,2)
    """
...
def all_angle_combinations(ind1,ind2):
    """
    Get all angles between all possible bonds also (1,2) and (17,20). Also shape (...,N) is possible.
